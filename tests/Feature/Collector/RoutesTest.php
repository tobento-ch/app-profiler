...
class RoutesTest extends \Tobento\App\Testing\TestCase
{
    ...
    public function testRoutesAreDisplayed()
    {
        $http = $this->fakeHttp();
        ...
        $http->response()
            ->assertStatus(200)
            ->assertNodeExists('a[href="#profiler-routes"]')
            ->assertBodyContains('Routes')
            ->assertBodyContains('POST')
            ->assertBodyContains('profiler/toolbar/profile')
            ->assertBodyContains('profiler.toolbar.profile')
            ->assertBodyContains('Tobento\App\Profiler\Controller\ToolbarController::profile');
