...
class BootsTest extends \Tobento\App\Testing\TestCase
{
    ...
    public function testBootsAreDisplayed()
    {
        $http = $this->fakeHttp();
        ...
        $http->response()
            ->assertStatus(200)
            ->assertBodyContains('Boots')
            ->assertBodyContains('Booted')
            ->assertBodyContains('Registered Boots')
            ->assertBodyContains('Tobento\App\Boot\Config');
